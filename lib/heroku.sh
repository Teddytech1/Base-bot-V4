...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║       TEDDY-XMD Heroku Deployer         ║"
echo "║         by Teddy             ║"
echo "╚═══════════════════════════════════════╝"
echo -e "${NC}"
...
APP_NAME=${APP_NAME:-teddy-xmd-bot}
...
OWNER_NUMBER=${OWNER_NUMBER:-254799963583}

# Bot name
read -p "$(echo -e ${CYAN}Bot name (default: MEGA-MD): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-TEDDY-XMD}
...
if [ ! -f "heroku.yml" ]; then
    echo -e "${YELLOW}📦 Cloning TEDDY-XMD repo...${NC}"
    git clone https://github.com/Teddytech1/TEDDY-XMD teddy-xmd-deploy
    cd teddy-xmd-deploy
fi
